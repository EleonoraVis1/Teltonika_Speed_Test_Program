...
double test_download_speed_by_id(int server_id) {
    ...
    if (!json_text) {
        printf("Could not read speedtest_server_list.json\n");
        return -1;
    }
    ...
    cJSON_ArrayForEach(item, servers) {
        ...
        if (id && cJSON_IsNumber(id) && id->valueint == server_id) {
            server = item;
            break;
        }
    }
    ...
    if (!server) {
        printf("Server not found for id=%d\n", server_id);
...
double upload_speed_test_by_id(int server_id, size_t upload_size_bytes) {
    ...
    if (!host_item || !cJSON_IsString(host_item)) {
        printf("Server host missing for id=%d\n", server_id);
        cJSON_Delete(servers);
        return -1;
    }
    ...
    while (1) {
        ...
        if (elapsed >= 1.0 && iterations >= 2) break;
    }
...
void test_best_server_for_country() {
    ...
    cJSON_ArrayForEach(item, servers) {
        ...
        // --- Check time limit after finishing the test ---
...
int main(int argc, char *argv[])
{
    ...
    if (n == 2) {
        int server_id = extra;
        test_download_speed_by_id(server_id);
    }

    if (n == 3) {
        int server_id = extra;
        size_t size = 3 * 1024 * 1024; // 3 MB
        upload_speed_test_by_id(server_id, size);
